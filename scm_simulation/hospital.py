...
class ParametricElectiveSurgeryDemandProcessWithTruncatedPoissonUsage(SurgeryDemandProcess):
    def __init__(self, seed=0):
        with open("model_artifact/simulation_inputs/historical_surgeries.pickle", "rb") as f:
            self.surgeries = list(pickle.load(f).values())

        with open("model_artifact/simulation_inputs/empirical_elective_surgery_distribution.pickle", "rb") as f:
            self.surgeries_per_day_sample = pickle.load(f)
        self.seed = seed
...
class ParametricEmergencySurgeryDemandProcessWithTruncatedPoissonUsage(SurgeryDemandProcess):
    def __init__(self, seed=0):
        ...
        with open("model_artifact/simulation_inputs/empirical_emergency_surgery_distribution.pickle", "rb") as f:
            self.surgeries_per_day_sample = pickle.load(f)
