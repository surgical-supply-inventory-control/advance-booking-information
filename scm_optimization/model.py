...
class PoissonUsageModel(UsageModel):
    def __init__(self, scale=1, extraParam=None, trunk=1e-5):
        self.name = "Poisson {} {}".format(str(scale), str(trunk))
        self.scale = scale
        self.trunk = trunk

    def usage(self, o,extraParam=None):
        return pacal.PoissonDistr(o * self.scale, trunk_eps=self.trunk)

    def random(self, o=1,extraParam=None):
        fail = True
        while fail:
            try:
...
class GenPoissonUsageModel(UsageModel):
    def __init__(self, scale=1, alpha = 0,trunk=1e-5):
        self.name = 'GenPoisson {} {} {}'.format(str(scale),str(alpha), str(trunk))
        self.scale = scale
        self.alpha = alpha
        self.trunk = trunk
    ...
    def usage(self,o, a ):
        ...
        if o>0:
            p = self.GenPoissonPdf(o*self.scale,a,0)
            usages.append(0)
            probs.append(p)

            for i in range(1,1000):
               p = self.GenPoissonPdf(o*self.scale,a,i)
               usages.append(i)
               if probs[-1]>p  and p<self.trunk:
                   probs.append(1-sum(probs))
                   break
               else:
                   probs.append(p)
            #print(usages,probs,self.scale,a)
            return pacal.DiscreteDistr(usages, probs)
        else:
            usages = [0]
            probs = [1]
            #print(usages,probs,self.scale,a)
    ...
    def random(self, o=1, a = 0):
        fail = True
        while fail:
            try:
                x = self.usage(o,a).rand(1)[0]
                fail = False
            except:
                fail = True
        return x
    ...
    def GenPoissonPdf(self,o=1,a=0,x=0):
        o = o*(1-a)
        return (o*(o+a*x)**(x-1))*(numpy.exp(-o-a*x))/math.factorial(x)
